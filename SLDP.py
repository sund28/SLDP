...
def trans_csvrow_to_gene_snp(row, gene_name='APOE'):  # gene 
    '''
    Input a row of the CSV file and return a Gene_SNP object. format of the row same as file under neighbor_snps_matched. 

    # TODO, note that A1 and A2 not always be ref and alt, need to check the ref and alt in the hg19. 
    '''
   
    start = gene_info[gene_name]['start']  # Start position of the gene, might need to be provided from elsewhere
    end = gene_info[gene_name]['end']  # End position of the gene, might need to be provided from elsewhere
    TSS = gene_info[gene_name]['TSS']  # Transcription start site, for - is end, 
    strand = gene_info[gene_name]['strand']  # Gene strand, might need to be provided from elsewhere

    chr = str(row['CHR'])  # Chromosome of the gene like '1'
    SNP_chr = str(row['CHR'])  # Assuming SNP is on the same chromosome as specified in 'CHR'
    SNP_pos = row['POS']
    SNP_ref = row['A2'] # ref is A2, still need to check alt and ref which is A1 and which is A2, and this is done in compute_and_save_gene_snp_perturb function, if it found wrong, will correct. 
    SNP_alt = row['A1'] # alt is A1 
    SNP_id = row['SNP']

    return Gene_SNP(gene_name, start, end, TSS, strand, chr, SNP_chr, SNP_pos, SNP_ref, SNP_alt, SNP_id)


def compute_and_save_gene_snp_perturb(gene_snp=gene_snp5):
    '''
    Compute the results for one gene_SNP pair, and save the (1, 896, 5313) matrix of ref and alt SNP of the gene to a h5 file. 
    gene_snp is a Gene_SNP object, used to compute the results.  
    position to save output is result_reference folder, file name is gene_name_SNP_id.h5. 
    '''
    ...
    ref_allele_hg19 = sequence_letter[snp_inds] # ref allele in hg19, double check it is ref allele, if not switch alt and ref allele. 
    if ref_allele_hg19 != ref: 
        temp = ref 
        ref = alt 
        alt = temp 
        print("switch ref and alt allele, as ref allele in hg19 is not ref allele in SNP file.")
    
    assert ref_allele_hg19 == ref #  "ref allele in hg19 should be the same as ref allele in SNP file."    
    ...
    '''
    output_path = f'/projects/zhanglab/users/dongbo/SLDP_project/result_reference/{gene_name}_{SNP_id}.h5'
...
if __name__ == "__main__":
    # TODO read each row read and drop those not TSS within 98034 of SNP. 
    snp_gene_list = '/projects/zhanglab/users/dongbo/SLDP_project/neighbor_snps_matched/rs2075620_APOE.csv'
    # snp_gene_list = '/projects/zhanglab/users/dongbo/SLDP_project/neighbor_snps_matched/rs2075620_CLASRP.csv' 
    # snp_gene_list = '/projects/zhanglab/users/dongbo/SLDP_project/neighbor_snps_matched/rs2155219_LRRC32.csv'
    # snp_gene_list = '/projects/zhanglab/users/dongbo/SLDP_project/neighbor_snps_matched/rs6690215_CR1.csv' 
    gene_name = 'APOE' # change gene name correspond to snp_gene_list. 
    # gene_name = 'CLASRP' # change gene name correspond to snp_gene_list. 
    # gene_name = 'LRRC32' # change gene name correspond to snp_gene_list. 
    # gene_name = 'CR1' # change gene name correspond to snp_gene_list. 

    for index, row in pd.read_csv(snp_gene_list).iterrows(): 
        gene_snp = trans_csvrow_to_gene_snp(row, gene_name=gene_name)  # gene 
        TSS = gene_snp.TSS 
        lower_bound = TSS - 98034
        upper_bound = TSS + 98034
        if not (row['POS'] >= lower_bound and row['POS'] <= upper_bound): 
            print("SNP not in TSS - 98034 and TSS + 98034 for gene: ",gene_snp.gene_name, "and SNP: ",gene_snp.SNP_id) 
            continue

        print(gene_snp.gene_name)
        print(gene_snp.SNP_id) 
        print(gene_snp.SNP_pos) 
        compute_and_save_gene_snp_perturb(gene_snp=gene_snp)     
    # print(gene_info)
    # print(gene_info[gene_name]['start']) 



    # compute_and_save_gene_snp_perturb(gene_snp=gene_snp) # test for one gene SNP pair. 